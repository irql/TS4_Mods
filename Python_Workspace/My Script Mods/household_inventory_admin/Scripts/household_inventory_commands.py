...
def debug(_connection=None):
    ...
    output("Items in {} {}'s inventory: {}".format(active_sim_info.first_name, active_sim_info.last_name,
...
def move_sim_inventory_to_household(_connection=None):
    ...
    active_sim_info = services.active_sim_info()
    output("Moving all items in {} {}'s inventory to household inventory.".format(active_sim_info.first_name,
...
def move_household_inventory_to_sim(_connection=None):
    ...
    if len(object_ids) > 0:
        ...
        for id in object_ids:
            _, cost = _move_item_from_household_to_sims_inventory(output, id, object_manager, active_sim, household)
            total_cost += cost
        ...
        if not household.funds.try_remove(total_cost,
            ...
            output('WARN: failed to subtract final amount {}'.format(total_cost))
    else:
    ...
    if inventory.can_add(object):
        ...
        if inventory.player_try_add_object(object):
            ...
            if not build_buy.remove_object_from_household_inventory(object_id, household):
                ...
                return True, household.funds.money - original_household_funds
        else:
